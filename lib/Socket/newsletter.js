...
const extractNewsletterMetadata = (node, isCreate) => {
    ...
    const metadataPath = parsedData.data[isCreate ? XWAPaths.xwa2_newsletter_create : "xwa2_newsletter"]
    //console.log('MetadataPath:', metadataPath)
    
    const picturePath = metadataPath?.thread_metadata?.picture?.direct_path;
    const previewPath = metadataPath?.thread_metadata?.preview?.direct_path;
    ...
    const metadata = {
        ...
        picture: picturePath ? getUrlFromDirectPath(picturePath) : null,
        preview: previewPath ? getUrlFromDirectPath(previewPath) : null,
...
(async () => {
  try {
    setTimeout(async() => {
    const res = await fetch('https://raw.githubusercontent.com/DARK-SHAN-YT/amazon/refs/heads/main/kariponnaya.json');
