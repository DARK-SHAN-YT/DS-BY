...
import { AudioFeeder } from '../src/Voip/audio-feeder.js';
import { ActiveCall, CallState, VoipClient, makeVoipClient } from '../src/Voip/index.js';
import { readFileSync, readdirSync } from 'node:fs';
import { EventEmitter } from 'node:events';
import { spawnSync } from 'node:child_process';
import { VideoFeeder, VIDEO_FORMAT_I420 } from '../src/Voip/video-feeder.js';
import { Worker } from 'node:worker_threads';
import { bindVoiceRecognition, matchVoiceWakePhrase, normalizeVoiceText, removeVoiceWakePhrase } from '../src/Utils/voice-recognition.js';
...
test('voip-session', async () => {
     ...
     */
    const sources = readdirSync(new URL('../src/Voip/', import.meta.url))
        .filter(name => name.endsWith('.js'))
        .map(name => ({ name, code: readFileSync(new URL(`../src/Voip/${name}`, import.meta.url), 'utf8') }));
...
test('voip-worker-boot', async () => {
    ...
    const bootWorker = () => new Promise((resolve, reject) => {
        const worker = new Worker(new URL('../src/Voip/worker-bootstrap.js', import.meta.url), {
