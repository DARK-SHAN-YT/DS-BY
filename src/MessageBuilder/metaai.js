...
import { AddonActionType } from './extras.js'
...
export const ClippyArtifactType = Object.freeze({ MINI_APP: 'MINI_APP', STATIC_HTML: 'STATIC_HTML' })
...
const ADDON_ACTION_UNSUPPORTED_PRIMITIVES = Object.freeze(['GenAIFooterActionPrimitive'])

export const addonActionSection = (primitives = [], { actionType, alignment = AddonActionAlignment.END } = {}) => {
    if (!Object.values(AddonActionType).includes(actionType)) {
        throw new TypeError('addonActionSection actionType must be one of ' + Object.values(AddonActionType).join(', '))
    }
    if (!Array.isArray(primitives) || primitives.length === 0) {
        throw new TypeError('addonActionSection needs at least one overlay button primitive; an empty AddonAction overlay force-closes the recipient')
    }
    for (const primitive of primitives) {
        if (primitive && ADDON_ACTION_UNSUPPORTED_PRIMITIVES.includes(primitive.__typename)) {
            throw new TypeError('addonActionSection cannot render ' + primitive.__typename + '; AddonAction is an overlay whose buttons need a label or icon, and this primitive leaves the button empty so the recipient force-closes. Forward a real Meta AI AddonAction with forwardRichResponse instead of synthesizing one')
        }
    }
    return layoutSection('AddonAction', primitives, { addon_action_type: actionType, addon_action_alignment: alignment })
}

export const QuotaUpsellMeterUsageType = Object.freeze({ META_AI_THINK_HARD: 'META_AI_THINK_HARD' })

export const quotaUpsellButton = ({ label, action, deeplink } = {}) =>
    typed('GenAIMetaSubsQuotaUpsellButton', { label, action, deeplink })

export const quotaUpsellSection = ({ title, body, bodyLine1, bodyLine2, meterUsageType, benefitType, buttons = [] } = {}) =>
    AIRich.newLayout('Single', typed('GenAIMetaSubsQuotaUpsellPrimitive', {
        title,
        body,
        body_line1: bodyLine1,
...
export const forwardRichResponse = async (sock, jid, msg, { quoted, contextInfo, messageId, additionalNodes = [], forwardWrapper = true, ...options } = {}) => {
    const signed = readSignedRichResponse(msg)
    if (!signed) {
