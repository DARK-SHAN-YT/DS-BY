...
export const makeSocket = (config) => {
    const { waWebSocketUrl, connectTimeoutMs, logger, keepAliveIntervalMs, browser, auth: authState, printQRInTerminal, defaultQueryTimeoutMs, transactionOpts, qrTimeout, pairingCodeTimeoutMs, makeSignalRepository } = config;
    ...
    const onWhatsAppUsername = async (...usernames) => {
        let usyncQuery = new USyncQuery().withContactProtocol();
        let added = 0;
        for (const entry of usernames) {
            const raw = typeof entry === 'string' ? entry : entry?.username;
            const username = typeof raw === 'string' ? raw.replace(/^@/, '').trim() : '';
            if (!username) {
                continue;
            }
            const user = new USyncUser().withUsername(username);
            const pin = typeof entry === 'object' && entry !== null ? entry.pin ?? entry.usernameKey : undefined;
    ...
    const ev = makeEventBuffer(logger, config.maxEventListeners);
    ...
    const clearPendingPairing = () => {
        if (pendingPairing?.timer) {
            clearTimeout(pendingPairing.timer);
        }
        pendingPairing = undefined;
    };

    const cancelPairingCode = () => {
        const wasPending = Boolean(pendingPairing);
        clearPendingPairing();
        return wasPending;
    };
    socketEndHandlers.push(clearPendingPairing);
    const requestPairingCode = async (phoneNumber, customPairingCode) => {
        if (pendingPairing) {
            const secondsLeft = Math.max(0, Math.ceil((pendingPairing.expiresAt - Date.now()) / 1000));
            throw new Boom('a pairing code is already pending; finish it, wait for it to expire, or call cancelPairingCode()', {
                statusCode: 409,
                data: { phoneNumber: pendingPairing.phoneNumber, secondsLeft }
            });
        }
        const normalizedNumber = String(phoneNumber ?? '').replace(/\D/g, '');
        if (normalizedNumber.startsWith('0') || normalizedNumber.length < 6 || normalizedNumber.length > 15) {
            throw new Boom('phoneNumber must be in international format: country code followed by the national number, digits only', {
                statusCode: 400,
                data: { phoneNumber }
            });
        }
        ...
        const guardMs = pairingCodeTimeoutMs + PAIRING_RESPONSE_GRACE_MS;
        const timer = setTimeout(() => {
            logger.info({ phoneNumber: normalizedNumber }, 'pairing code expired without being used');
            pendingPairing = undefined;
        }, guardMs);
        timer.unref?.();
        pendingPairing = { phoneNumber: normalizedNumber, expiresAt: Date.now() + pairingCodeTimeoutMs, timer };
        authState.creds.pairingCode = pairingCode;
        authState.creds.me = {
            id: jidEncode(normalizedNumber, 's.whatsapp.net'),
            name: '~'
        };
        ...
        if (!response) {
            clearPendingPairing();
            throw new Boom('the server never answered the pairing request', {
                statusCode: DisconnectReason.timedOut,
                data: { phoneNumber: normalizedNumber }
            });
        }
        const registration = getBinaryNodeChild(response, 'link_code_companion_reg');
        const pairingRef = registration && getBinaryNodeChild(registration, 'link_code_pairing_ref');
        if (!pairingRef?.content) {
            clearPendingPairing();
            throw new Boom('the server accepted the pairing request without registering it', {
                statusCode: 500,
                data: { response: binaryNodeToString(response) }
            });
        }
        logger.info({
            phoneNumber: normalizedNumber,
            refLength: pairingRef.content.length,
            stage: registration.attrs?.stage
        }, 'pairing code registered with the server');
    ...
    const fetchNewChatMessageCap = async () => {
        ...
        return {
            ...
            oteStatus: response?.ote_status,
            mvStatus: response?.mv_status,
