...
const defaultsChanged = update('src/Defaults/index.js', /const\s+version\s*=\s*\[\s*\d+\s*,\s*\d+\s*,\s*\d+\s*\]\s*;/, `const version = ${value};`)
const genericsChanged = update('src/Utils/generics.js', /const\s+baileysVersion\s*=\s*\[\s*\d+\s*,\s*\d+\s*,\s*\d+\s*\]\s*;/, `const baileysVersion = ${value};`)
